...
public class Sudoku {
	...
	*/
     ...
     *     checkField which is not set if
     *         onePossibiliy then valueChanged to true and exit loop
     *         noPossibility then notSolvable to true and exit loop
     *   if notSolvable true then return
     * 
     */
	do {
	    valueChanged = false;
		...
		for (int j=0; j<9; j++) {
		    // j is counting the columns
		    //SudokuField tempField = new SudokuField();
		    // tempField = this.sudokuArray[i][j];
			...
			if (this.sudokuArray[i][j].hasNoSolution() ) {
			    solvable = false;
			}
		    } 
...
/* try with solvable only
*	} while (valueChanged &&  solvable );
*/
	} while (valueChanged);
	return solvable;
    }
